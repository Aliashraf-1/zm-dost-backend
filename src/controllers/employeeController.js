...
exports.getEmployees = async (req, res) => {
  try {
    // Non-admin users may only load their own employee record. This keeps the
    // self-service employee, tasks, and lead views from exposing other staff.
    const employees = isAdmin(req.user)
      ? await Employee.find()
      : await Employee.find({ email: req.user.email?.toLowerCase() });
    res.status(200).json({
      success: true,
...
exports.getEmployee = async (req, res) => {
  try {
    ...
    if (!isAdmin(req.user) && employee.email?.toLowerCase() !== req.user.email?.toLowerCase()) {
...
exports.createEmployee = async (req, res) => {
  try {
    ...
    // System settings provide defaults for new employees; values submitted in
    // the employee form are retained as that employee's custom overrides.
    const settings = await Setting.findOne();
    const defaults = settings?.employee || {};
    employeeData.shiftTiming = {
      graceMinutes: defaults.graceMinutes ?? 30,
      weeklyOff: defaults.weeklyOffDay ?? 'Friday',
      monthlyLeaves: defaults.monthlyPaidLeaves ?? 1,
      ...(employeeData.shiftTiming || {}),
    };
    employeeData.attendanceSettings = {
      leaveDeduction: defaults.defaultLeaveDeduction ?? 500,
      lateDeduction: defaults.defaultLateDeduction ?? 10,
      taskFailureDeduction: defaults.defaultTaskFailureDeduction ?? 1000,
      ...(employeeData.attendanceSettings || {}),
    };
  ...
  try {
    ...
    if (employee.status !== 'Active') {
      return res.status(400).json({
        success: false,
        message: 'Attendance can only be marked for active employees.',
      });
    }
    ...
    if (employee.joiningDate && date < employee.joiningDate) {
      return res.status(400).json({
        success: false,
        message: 'Attendance cannot be marked before the employee joining date.',
      });
    }
...
exports.paySalary = async (req, res) => {
  try {
    ...
    if (employee.status !== 'Active') {
      return res.status(400).json({
        success: false,
        message: 'Salary cannot be recorded for an inactive employee.',
      });
    }

    const salaryMonth = month || new Date().toISOString().slice(0, 7);
    if (employee.joiningDate && salaryMonth < employee.joiningDate.slice(0, 7)) {
      return res.status(400).json({
        success: false,
        message: 'Salary cannot be recorded before the employee joining month.',
      });
    }
